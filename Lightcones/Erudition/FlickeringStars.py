...
class FlickeringStars(Lightcone):
    ...
    def equip(self):
        ...
        SKLDmgAount = self.level * 0.06 + 0.30
    ...
    def ownTurn(self, turn: Turn, result: Result):
        ...
        TeamShred = self.level * 0.02 + 0.08
        SelfShred = self.level * 0.03 + 0.17
        if turn.moveName == "ArcherSkill" or (turn.moveName == "RinTohsakaSkillSingle" and turn.spChange <= -4):
            ...
            bl.append(Buff("FlickeringStarsSelfShred", StatTypes.SHRED, SelfShred, self.wearerRole, [AtkType.ALL], 3, 1,Role.SELF, TickDown.END))
        return bl, dbl, al, dl, hl
