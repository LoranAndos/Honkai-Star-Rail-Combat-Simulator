...
class UntilTheFlowersBloomAgain(Lightcone):
    ...
    def equip(self):
        ...
        if self.level <= 3:
            ERRBuff = floor(self.level * 0.015 + 0.085)
        elif self.level == 4:
            ERRBuff = 0.14
        else:
            ERRBuff = 0.16
        ...
        if self.maxEnergy <= 120:
            ...
            bl.append(Buff("FlowersBloomExtraERR", StatTypes.ERR_PERCENT, ERRExtraBuff, self.wearerRole, [AtkType.ALL], 1, 1, Role.SELF, TickDown.PERM))
        return bl, dbl, al, dl, hl

    def useElaSkill(self, enemyID = -1):
        bl, dbl, al, dl, hl = super().equip()
        VulAmount = self.level * 0.375 + 0.1125
